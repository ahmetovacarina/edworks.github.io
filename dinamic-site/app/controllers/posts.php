...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_post'])){
    if(!empty($_FILES['img']['name'])){
       ...
       if (strpos($fileType, 'image') === false){
            array_push ($errMsg,'Можно загрузить только изображения.');
       }else{
    ...
    if($title === '' || $content === '' || $topic === ''){
        array_push($errMsg,'Не все поля не заполнены!');
    }elseif (mb_strlen($title, 'UTF8') < 2){
        array_push($errMsg,'Название поста должно быть более 2-х символов');
    } else{
        $post = [
            'id_user' => $_SESSION['id'],
            'title' => $title,
